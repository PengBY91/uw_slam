...
`sonar_cfar_frontend` — it is never linked into `include/`/`src/` (see the `sonar_cfar_frontend`
entry in `/NOTICE` for what geometry *was* ported natively, now under `include/frontends`/
`src/frontends`, which is a separate, deliberate exception limited to CFAR/DBSCAN/polar math, not
the ROS node itself).
...
3. Record `/sonar_camera_reconstruction/cloud` and hand it to this platform's evaluation tooling
   (`include/evaluation`/`src/evaluation`) alongside this platform's native map output, for an
