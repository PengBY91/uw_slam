...
the SVIn bridge, `adapters/svin_bridge.md`): wraps HoloOcean's official ROS 2
interface (`external_repos/holoocean-ros`, byu-holoocean/holoocean-ros, MIT)
as a `SonarFrameProvider` (see `include/measurement_api/providers.hpp`),
...
the external baseline path (`baselines/sonar_camera_reconstruction`)
consumes instead.

`HoloOceanRosBridgeSonarFrameProvider` (see
[`include/adapters/holoocean_ros_bridge_sonar_frame_provider.hpp`](../include/adapters/holoocean_ros_bridge_sonar_frame_provider.hpp))
