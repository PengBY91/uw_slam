...
`adapters/holoocean` and `apps/tools/synth_bag_gen` produce, so `apps/replay_demo` and the rest of the
...
When adding one, follow the pattern in `apps/tools/synth_bag_gen/src/main.cpp` (topics, message types,
