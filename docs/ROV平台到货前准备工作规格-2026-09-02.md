...
> 状态：第 3 周**已完整执行并通过全部验收（SITL 侧 + 真实 HoloOcean/UE5 侧）**（2026-09-03）——结论文档 `adapters/holoocean/docs/ardusub-sitl-bridge-feasibility.md`。**方案甲可行**：整条闭环在真实 HoloOcean 上跑通，稳态 **50.0 frames/s / RTF 1.00**，全程 stale/dropped/foreign 全 0，SITL 接受回传 JSON 的六个必需字段，深度传感器 spawn 3.00 m 实读 3.07 m，四轴响应 forward +2.095 m/s、right +2.095 m/s（与 forward 完全对称）、down +4.594 m/s、yaw +0.984 rad/s（受控），DEPTH_HOLD 15 s 保持 0.32 m，EKF 与物理真值吻合。并首次实测证实 HoloOcean 的 IMUSensor **含重力**（首帧 |accel| = 9.800 m/s²）。规格预设的退路方案乙**不需要启用**。
