...
> 状态：草案 v1.2 · 2026-09-02 · 待技术负责人定案
> 输入依据：`docs/ROV平台参数.md`（合同技术附件转写）、团队决策（HWT9053-485 外挂 IMU；供应商不做任何算法优化；**后续会另行挂载一套双目相机**，型号与时间待定；**DVL 后期也不加装**）
> 修订：v1.1 2026-09-02——D-1 改为分阶段表述，A-03 改为单/双目可配，新增 PREP-B-08 双目上机准备与双目带宽策略；v1.2 2026-09-02——PREP-B-03 展开为六步实施方案，新增 PREP-A-13 设备伪装层，DVL 排除，PREP-F-01 已执行，全文审校（任务计数、排期、若干技术细节修正）
...
> 状态：第 3 周**已完整执行并通过全部验收（SITL 侧 + 真实 HoloOcean/UE5 侧）**（2026-09-03）——结论文档 `adapters/holoocean/docs/ardusub-sitl-bridge-feasibility.md`。**方案甲可行**：整条闭环在真实 HoloOcean 上跑通，稳态 **50.0 frames/s / RTF 1.00**，全程 stale/dropped/foreign 全 0，SITL 接受回传 JSON 的六个必需字段，深度传感器 spawn 3.00 m 实读 3.07 m，四轴响应 forward +2.095 m/s、right +2.095 m/s（与 forward 完全对称）、down +4.594 m/s、yaw +0.984 rad/s（受控），DEPTH_HOLD 15 s 保持 0.32 m，EKF 与物理真值吻合；**30 分钟连续运行 89802 帧 / 1796 s 仿真时间，stale=0 dropped=0 foreign=0，规格的"30 分钟不失步"通过**（中途出现一次 WSL2↔Windows 的 TCP `ConnectionResetError`，桥接自动重连、仿真时间无跳变、SITL 全程未察觉——这条链路在真实网络抖动下能自愈）。并首次实测证实 HoloOcean 的 IMUSensor **含重力**（首帧 |accel| = 9.800 m/s²）。规格预设的退路方案乙**不需要启用**。
> **本次最重要的发现（影响面超出 A-05）：HoloOcean Python 客户端 `agents.py` 里的 BlueROV2 推进器表是错的**（上游自己标注"may not be correct — check the C++"，核对引擎 `BlueROV2.h/.cpp` 后确认：位置 x 号全部取反、斜推方向错——引擎四个斜推**全部朝前**而 Python 表是前两个朝后）。本仓库 `adapters/holoocean/uw_holoocean_adapter/thrust_allocation.py` 一直基于这张错表，而 `allocate()` 是它的伪逆、驱动 `scripted_pilot.py` 的飞手指令通路（PREP-C-02）。用错表在引擎里的实际效果：**纯 surge 指令产生零净力**、偏航与俯仰反向（lateral/throttle/roll 碰巧正确）。已改用引擎 C++ 真值并补了两条本该抓到它的测试（此前 9 条分配单测全是 `allocate → commanded_wrench` 的自洽性检查，对任何内部一致的表都成立）。**需要清点此前所有基于 HoloOcean 的操纵/控制观察**——"给前进指令但机体不太动"这类现象都可能是这个 bug 而非水动力。
> **一条已撤回的结论**：本文档 v2 曾报告"HoloOcean 的 BlueROV2 几乎没有偏航阻尼（≈0.12 N·m·s/rad）"并判定影响 PREP-A-12/C-07/C-09。**撤回**——62 rad/s 的自转不是欠阻尼开环响应，而是偏航反接把 ArduSub 的航向保持变成了正反馈；修正几何后同一套检查测到 0.98 rad/s 受控偏航。引擎的转动阻尼确实偏弱（只有 UE 的一阶 `SetAngularDamping(0.75)`，无水动力转动阻力）、平动阻力各向同性（下潜能到 4.6 m/s，真机约 1 m/s 量级），这是保真度限制、不是阻碍，但在仿真里调垂向/姿态参数前要先决定是否可接受。
...
> 过程中定位到 7 项事实，其中两项是**第 1 周 PREP-C-01 的遗留问题**：(1) ArduSub 的 SITL 气压计是水压计、深度取 AMSL 海拔的相反数，`sim_vehicle.py` 默认 home 海拔 584 m 使 `press_abs` 一直是 −55925 hPa（`sitl_smoke.py` 打印了但没校验）；(2) `SIM_RATE_HZ` 默认 1200 而非 readme 写的 400。另外发现 **HoloOcean 自己的 BlueROV2 推进器表在偏航上不对称**（后侧两个斜推力臂只有前侧的 1/6），纯横移会产生大的寄生偏航——这条影响 `thrust_allocation.py`（PREP-C-02）的分配矩阵是否标定正确，是需要在 HoloOcean 上实测拍板的开放问题。**未做**：QGC 操控录屏（阻塞已解除——机体现在行为正常，但录屏要在 Windows 上人工做：QGC 连 `tcp:<WSL2 IP>:5760`）、加回感知后的 RTF。
...
- **现状**：`schemas/proto/uw/domain/measurement.proto` 已定义 `ImuPreintegrationMeasurement`（第 76 行起）；`imu.proto` 有 `ImuSample`；rig `imu_noise` 四参数已有；`include/frontends/`、`include/factor_builders/` 下**没有**任何 IMU 实现；`GaussNewtonSolver` 状态目前只有 `Pose3`，没有速度和偏置状态。
- **做法**：
...
| 第 5 周 | PREP-C-08 系统辨识方案；PREP-A-13 外部时钟源模式 | PREP-C-03（外部导航回灌）+ PREP-C-05/C-06 | **PREP-B-03 声呐配准里程计（第 1–3 步）**——阶段 1 唯一的长时漂移抑制手段，提前到关卡建模之前 | PREP-B-06 实现 |
