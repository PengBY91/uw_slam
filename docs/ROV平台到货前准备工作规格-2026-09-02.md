...
本文档把这些准备工作拆成 6 个工作包（WP-A ～ WP-F）、共 35 项任务，每项给出目标、现状、具体做法、涉及文件、验收标准、依赖和估算工时。任务编号 `PREP-<WP>-<nn>`，供路线图和后续 traceability 引用。角色标签沿用路线图第 7 节：`技术负责人` / `SDK/平台集成` / `光学与任务感知` / `HMI 与系统验证`。
...
- **目标**：相邻声呐帧之间的 2D（x、y、yaw）相对位姿估计，作为 IMU 预积分之外的第二个相对位姿来源，抑制长时漂移。
- **现状**：`sonar_cfar_frontend` 只做检测；无配准。
- **做法**：
  1. 第一版用**极坐标图像的相位相关**（range 方向对数极坐标 + 方位方向，Fourier-Mellin 变体）估平移和旋转，粗略但鲁棒、无特征依赖；输出 `RelativePoseMeasurement`（已有 proto）加协方差（由相关峰尖锐度估）。
  2. 第二版（可选）在 CFAR 点集上做 ICP 精化。
  3. `include/frontends/sonar_registration_frontend.hpp` + `src/…`；测试用 HoloOcean 声呐录制加真值。
  4. 与现有 `relative_pose_factor_builder` 对接，不新建 factor builder。
- **验收**：HoloOcean 基线 60 s 段，纯声呐配准里程计 ATE ≤1 m；与 IMU 预积分联合后优于任一单独来源。
- **依赖**：PREP-A-03。**工时**：8 人日。**角色**：`光学与任务感知`。
...
- **验收**：SITL 下，喂一条已知合成轨迹的增量序列，SITL 回读 `LOCAL_POSITION_NED` 与合成轨迹误差 <5%；POSHOLD 模式在无 GPS/DVL 下可切入并保持；断开增量后 EKF 在参数指定超时内退化、模式自动回落且日志有记录。
...
- **验收**：`live_ingress_smoke_test.sh` 加一条用回放 UDP 流的用例通过。
...
- **验收**：用 SITL 环境下的假 gstreamer 源验证解码与时间戳。**工时**：1.5 人日。**角色**：`光学与任务感知`。
...
| 第 4 周 | PREP-B-01 收尾 + 端到端 demo；PREP-B-02 航向因子 | PREP-C-03 MAVLink adapter（遥测进 + 命令出） | PREP-A-06 续；PREP-A-09 外观退化 | PREP-A-08 水池关卡；PREP-A-12 飞手训练台 |
| 第 5 周 | PREP-C-08 系统辨识方案 | PREP-C-03（外部导航回灌）+ PREP-C-05/C-06 | PREP-A-07 结构物关卡；PREP-B-07 单目检测路径 | PREP-B-06 实现 |
| 第 6 周 | PREP-C-09 相对定点控制律 | PREP-D-03/D-04；PREP-E-03 部署拓扑；PREP-C-07 | PREP-B-03 声呐配准里程计 | PREP-A-11 合成数据 |
| 第 7–8 周 | PREP-C-09 续；PREP-E-05；PREP-B-08 阶段 2 组合模式 | PREP-B-04 声呐 adapter 骨架；PREP-E-04 相机流；PREP-F-03 验收清单定稿 | PREP-B-03 续；PREP-E-01 带宽退化曲线；PREP-B-08 仿真退化表 | 端到端仿真闭环回归；规格/traceability 同步 |
